...
abstract public class Instruction implements Util {
  ...
  Instruction(JComet2 machine, int opcode, String opname, ArgType argtype) {
    this.m = machine;
    this.opcode = opcode;
    this.opname = opname;
  ...
  private String disassembleAdrX(int address) {
    ...
    int adr = adrx[0];
    int x = adrx[1];
    return x == 0 ? String.format("%-8s#%04x", opname, adr)
        : String.format("%-8s#%04x, GR%1d", opname, adr, x);
  }
  ...
  private String disassembleRAdrX(int address) {
    ...
    int adr = radrx[1];
    int x = radrx[2];
    return x == 0 ? String.format("%-8sGR%1d, #%04x", opname, r, adr)
        : String.format("%-8sGR%1d, #%04x, GR%1d", opname, r, adr, x);
  }
