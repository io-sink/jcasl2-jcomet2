...
public class JComet2 implements Util {
  ...
  private static class StatusMonitor {
    ...
    public String toString() {
      return String.format("%04d: ", m.stepCount) + this.varList.stream()
          .map(Supplier::get)
    ...
    public void append(String s) {
      try {
        ...
        System.err
            .println(String.format("Warning: Invalid monitor target is found. %s is ignored.", s));
      }
    }
  }
  ...
  private JComet2() {
    List<Instruction> instList = Arrays.asList(new NOP(this), new LD2(this), new ST(this),
        new LAD(this), new LD1(this), new ADDA2(this), new SUBA2(this), new ADDL2(this),
        new SUBL2(this), new ADDA1(this), new SUBA1(this), new ADDL1(this), new SUBL1(this),
        new AND2(this), new OR2(this), new XOR2(this), new AND1(this), new OR1(this),
        new XOR1(this), new CPA2(this), new CPL2(this), new CPA1(this), new CPL1(this),
        new SLA(this), new SRA(this), new SLL(this), new SRL(this), new JMI(this), new JNZ(this),
        new JZE(this), new JUMP(this), new JPL(this), new JOV(this), new PUSH(this), new POP(this),
        new CALL(this), new RET(this), new SVC(this), new IN(this), new OUT(this), new RPUSH(this),
        new RPOP(this));
  ...
  public void printStatus() {
    String code;
    try {
      code = this.getInstruction()
          .disassemble(this.PR);
    } catch (Exception e) {
      code = String.format("%04x", this.memory[this.PR]);
    }
    System.err
        .println(String.format("PR  #%04x [ %-30s ]  STEP %d", this.PR, code, this.stepCount));
    System.err.println(String.format("SP  #%04x(%7d) FR(OF, SF, ZF)  %3s  (%7d)", this.getSP(),
        this.getSP(), this.getFRasString(), this.getFR()));
    System.err.println(String.format("GR0 #%04x(%7d) GR1 #%04x(%7d) GR2 #%04x(%7d) GR3: #%04x(%7d)",
        this.GR[0], l2a(this.GR[0]), this.GR[1], l2a(this.GR[1]), this.GR[2], l2a(this.GR[2]),
        this.GR[3], l2a(this.GR[3])));
    System.err.println(String.format("GR4 #%04x(%7d) GR5 #%04x(%7d) GR6 #%04x(%7d) GR7: #%04x(%7d)",
        this.GR[4], l2a(this.GR[4]), this.GR[5], l2a(this.GR[5]), this.GR[6], l2a(this.GR[6]),
  ...
  private String dumpMemory(int startAddr, int lines) {
    ...
    for (int i = 0; i < lines; i++) {
      ...
      sb.append(String.format("%04x: %-39s %-8s%n", addr,
  ...
  private void disassemble(int startAddr) {
    ...
    for (int i = 0; i < 16; i++) {
      try {
        ...
        if (inst != null) {
          System.out.println(
              String.format("#%04x\t#%04x\t%s", addr, this.memory[addr], inst.disassemble(addr)));
  ...
  public static void execute(final File inputFile, final File outputFile, final String... args) {
    try {
      ...
      final JComet2 comet2 = new JComet2();
      comet2.out = out;
      ...
      comet2.run();
      Files.write(outputFile.toPath(), out.toString()
  ...
  public static void main(String[] args) {
    ...
    if (version) {
      ...
      System.out.println("Usage: " + JComet2.class.getSimpleName() + " [options] input.com");
      System.out.println(options);
    } else {
      JComet2 comet2 = new JComet2();
